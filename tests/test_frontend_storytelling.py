...
class FrontendStorytellingTests(unittest.TestCase):
    ...
    def test_operations_distinguishes_replay_from_demo_telemetry(self):
        ...
        self.assertIn("Captured run", self.javascript)
        self.assertIn("run.completedAt", self.javascript)
        self.assertIn("run.displayLabel || run.mode", self.javascript)
        self.assertNotIn('class="run-mode', self.javascript)
        self.assertNotIn("const modeLabel", self.javascript)
        self.assertNotIn("const capturedAt", self.javascript)
